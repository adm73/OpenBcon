...
function buildDeploymentEnvTemplate(draft: DeploymentWizardDraft) {
  const domain = draft.domain.trim() || 'your-domain.example'
...
# Proxy plan: ${draft.proxy === 'caddy' ? 'Caddy terminates HTTPS directly on ports 80/443.' : 'An existing Traefik owns ports 80/443 and forwards the configured host to OpenBcon.'}
`
}
...
export function AdminPage() {
  ...
  function validateDeploymentStep(step: DeploymentWizardStep) {
    if (step === 1 && !isValidDeploymentDomain(deploymentDraft.domain)) {
      setDeploymentNotice(
        'Enter a valid domain such as client.example.com. Do not include https://, a path, or a port.',
      )
      return false
    }
              ...
              {deploymentStep === 1 ? (
                <div className="admin-deployment-panel">
                  <div>
                    ...
                    <h3>What domain will this deployment use?</h3>
                    <p>
                      Enter the public hostname customers will use. The domain must
                      already point to the server before HTTPS can be issued.
                    </p>
                  </div>
                  <label>
                    <span>Domain</span>
                    <input
                      value={deploymentDraft.domain}
                      onChange={(event) => updateDeploymentDraft('domain', event.target.value)}
                      placeholder="client.example.com"
                      autoComplete="url"
                      spellCheck={false}
                    />
                    <small>Use a hostname only. Example: reports.client.com</small>
                  </label>
                </div>
              ) : null}
                ...
                <div className="admin-deployment-panel">
                  ...
                  <div className="admin-deployment-choice-grid">
                    ...
                    <label className={deploymentDraft.proxy === 'traefik' ? 'is-selected' : ''}>
                      ...
                      <small>Traefik owns 80/443 and forwards this domain internally.</small>
                    </label>
                  </div>
                  <label>
              ...
              {deploymentStep === 4 ? (
                <div className="admin-deployment-panel admin-deployment-review">
                  <div>
                    ...
                    <p>
                      The domain and proxy plan below are used to create a server-side
                      environment template. Review it before copying it to the VPS.
                    </p>
                  </div>
                  <dl className="admin-deployment-summary">
                    <div><dt>Domain</dt><dd>{deploymentDraft.domain}</dd></div>
