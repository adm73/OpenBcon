...
type DeploymentWizardStep = 1 | 2 | 3 | 4
type DeploymentProxy = 'caddy' | 'traefik'
...
const deploymentWizardStorageKey = 'openbcon-deployment-wizard-v1'
const defaultDeploymentWizardDraft: DeploymentWizardDraft = {
  publicDomain: '',
  dashboardDomain: '',
  certificateEmail: '',
  proxy: 'caddy',
...
function loadDeploymentWizardDraft(): DeploymentWizardDraft {
  if (typeof window === 'undefined') return defaultDeploymentWizardDraft

  try {
    const parsed = JSON.parse(
      window.localStorage.getItem(deploymentWizardStorageKey) ?? '{}',
    ) as Partial<DeploymentWizardDraft>
    const legacyDomain = typeof (parsed as { domain?: unknown }).domain === 'string'
      ? (parsed as { domain: string }).domain
      : ''
    return {
      ...defaultDeploymentWizardDraft,
      publicDomain:
        typeof parsed.publicDomain === 'string' ? parsed.publicDomain : legacyDomain,
      dashboardDomain:
        typeof parsed.dashboardDomain === 'string'
          ? parsed.dashboardDomain
          : legacyDomain
            ? `dashboard.${legacyDomain}`
            : '',
      certificateEmail:
        typeof parsed.certificateEmail === 'string' ? parsed.certificateEmail : '',
      proxy: parsed.proxy === 'traefik' ? 'traefik' : 'caddy',
      environmentMode: parsed.environmentMode === 'live' ? 'live' : 'test',
    }
  } catch {
    return defaultDeploymentWizardDraft
  }
}

function isValidDeploymentDomain(value: string) {
  const domain = value.trim().toLowerCase()
  if (domain === 'localhost') return true
  return /^(?=.{1,253}$)(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,63}$/u.test(
    domain,
  )
}

function isValidDeploymentEmail(value: string) {
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/u.test(value.trim())
}

function buildDeploymentEnvTemplate(draft: DeploymentWizardDraft) {
  const publicDomain = draft.publicDomain.trim() || 'your-domain.example'
  const dashboardDomain = draft.dashboardDomain.trim() || `dashboard.${publicDomain}`
  const email = draft.certificateEmail.trim() || 'admin@example.com'
  const publicUrl = publicDomain === 'localhost' ? 'http://localhost:5173' : `https://${publicDomain}`
  const dashboardUrl = dashboardDomain === 'localhost'
    ? 'http://localhost:5173'
    : `https://${dashboardDomain}`

  return `# OpenBcon deployment settings generated by Deployment Setup
# Keep this file on the server. Do not commit it to Git.
...
PUBLIC_APP_URL=${dashboardUrl}
CORS_ORIGIN=${publicUrl},${dashboardUrl}
OPENBCON_ENVIRONMENT_MODE=${draft.environmentMode}

# Keep these values server-side and replace every placeholder before starting:
POSTGRES_PASSWORD=replace_with_a_long_random_password
MONGODB_ROOT_PASSWORD=replace_with_a_long_mongo_password
APP_STATE_ENCRYPTION_KEY=replace_with_openssl_rand_hex_32
OPENBCON_OPENAI_API_KEY=replace_with_a_server_side_key

# Proxy plan: ${draft.proxy === 'caddy' ? 'Caddy terminates HTTPS directly on ports 80/443.' : 'An existing Traefik owns ports 80/443 and forwards both configured hosts to OpenBcon.'}
`
}
...
export function AdminPage() {
  ...
  const [deploymentStep, setDeploymentStep] = useState<DeploymentWizardStep>(1)
  const [deploymentDraft, setDeploymentDraft] = useState<DeploymentWizardDraft>(
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(
        deploymentWizardStorageKey,
        JSON.stringify(deploymentDraft),
      )
    } catch {
      // Deployment drafts are a convenience; a restricted browser must not block Admin Console.
    }
  }, [deploymentDraft])
  ...
  function updateDeploymentDraft<Key extends keyof DeploymentWizardDraft>(
    field: Key,
    value: DeploymentWizardDraft[Key],
  ) {
  ...
  function validateDeploymentStep(step: DeploymentWizardStep) {
    if (step === 1 && !isValidDeploymentDomain(deploymentDraft.publicDomain)) {
      setDeploymentNotice(
        'Enter a valid public domain such as client.example.com. Do not include https://, a path, or a port.',
      )
      return false
    }
    if (step === 1 && !isValidDeploymentDomain(deploymentDraft.dashboardDomain)) {
      setDeploymentNotice(
        'Enter a valid dashboard domain such as dashboard.client.com. Do not include https://, a path, or a port.',
      )
      return false
    }
    if (step === 2 && !isValidDeploymentEmail(deploymentDraft.certificateEmail)) {
      setDeploymentNotice('Enter a valid email address for automatic HTTPS certificate notices.')
      return false
    }
    return true
  }

  function advanceDeploymentStep() {
    if (!validateDeploymentStep(deploymentStep)) return
    setDeploymentStep((current) => (current < 4 ? (current + 1) as DeploymentWizardStep : current))
  }

  function resetDeploymentWizard() {
    setDeploymentDraft(defaultDeploymentWizardDraft)
    setDeploymentStep(1)
    setDeploymentNotice('Deployment draft cleared from this browser.')
    setDeploymentCopied(false)
  }

  async function copyDeploymentCommand() {
    const command = `cp deploy/.env.production.example deploy/.env.production\n# edit deploy/.env.production, then run:\n./deploy/deploy.sh`
    try {
      await navigator.clipboard.writeText(command)
      setDeploymentCopied(true)
      setDeploymentNotice('Deployment commands copied. Run them on the server after editing the environment file.')
    } catch {
      setDeploymentNotice('Clipboard access was blocked. Select and copy the commands manually.')
    }
  }

  function downloadDeploymentEnv() {
    const blob = new Blob([buildDeploymentEnvTemplate(deploymentDraft)], {
      type: 'text/plain;charset=utf-8',
    })
    const url = URL.createObjectURL(blob)
    const anchor = document.createElement('a')
    anchor.href = url
    anchor.download = '.env.production.template'
    document.body.appendChild(anchor)
    anchor.click()
    anchor.remove()
    URL.revokeObjectURL(url)
    setDeploymentNotice('Environment template downloaded. Fill in secrets on the server before deployment.')
  }
    ...
    <div className="admin-shell">
      <aside className="admin-rail">
        ...
        <nav>
          ...
          <a href="#deployment">Deployment Setup</a>
      ...
      <main className="admin-main">
        <form className="admin-form" onSubmit={saveSettings}>
          ...
          <section className="admin-card admin-management-card" id="deployment">
            <div className="admin-section-copy">
              <p className="admin-section-number">02</p>
              <h2>Deployment Setup</h2>
              <p>
                Prepare OpenBcon for any domain. This wizard creates a safe server
                configuration template; it does not execute commands on a VPS.
              </p>
            </div>
            <div className="admin-management-content admin-deployment-wizard">
              <div className="admin-deployment-steps" aria-label="Deployment setup steps">
                {[
                  ['01', 'Domain'],
                  ['02', 'HTTPS'],
                  ['03', 'Runtime'],
                  ...
                  const step = (index + 1) as DeploymentWizardStep
                  return (
                    <button
                      key={number}
                      type="button"
                      className={deploymentStep === step ? 'is-active' : ''}
                      onClick={() => {
                        if (step <= deploymentStep) setDeploymentStep(step)
                      }}
                      disabled={step > deploymentStep}
                    >
                      <b>{number}</b>
                      <span>{label}</span>
                    </button>
              ...
              {deploymentStep === 1 ? (
                <div className="admin-deployment-panel">
                  <div>
                    <span className="admin-deployment-eyebrow">Step 01</span>
                    <h3>Which domains will this deployment use?</h3>
                    <p>
                      OpenBcon uses one public hostname for the website and one dashboard
                      hostname for sign-in and the workspace. Both domains must point to
                      this server before HTTPS can be issued.
                    </p>
                  </div>
                  <label>
                    <span>Public site domain</span>
                    <input
                      value={deploymentDraft.publicDomain}
                      onChange={(event) => updateDeploymentDraft('publicDomain', event.target.value)}
                      placeholder="client.example.com"
                      autoComplete="url"
                      spellCheck={false}
                    />
                    <small>Website and public program directory. Example: client.com</small>
                  </label>
                  <label>
                    <span>Dashboard domain</span>
                    <input
                      value={deploymentDraft.dashboardDomain}
                      onChange={(event) => updateDeploymentDraft('dashboardDomain', event.target.value)}
                      placeholder="dashboard.client.com"
                      autoComplete="url"
                      spellCheck={false}
                    />
                    <small>Sign-in, workspace, and admin console. Example: dashboard.client.com</small>
                  </label>
                </div>
              ) : null}
              ...
              {deploymentStep === 2 ? (
                <div className="admin-deployment-panel">
                  <div>
                    <span className="admin-deployment-eyebrow">Step 02</span>
                    <h3>Choose HTTPS and proxy ownership</h3>
                    <p>
                      Let&apos;s Encrypt is automatic. Only one service can own ports
                      80 and 443, so choose the arrangement that matches the VPS.
                    </p>
                  </div>
                  <div className="admin-deployment-choice-grid">
                    <label className={deploymentDraft.proxy === 'caddy' ? 'is-selected' : ''}>
                      <input
                        type="radio"
                        name="deployment-proxy"
                        checked={deploymentDraft.proxy === 'caddy'}
                        onChange={() => updateDeploymentDraft('proxy', 'caddy')}
                      />
                      <strong>Caddy direct</strong>
                      <small>OpenBcon Caddy terminates HTTPS on ports 80/443.</small>
                    </label>
                    <label className={deploymentDraft.proxy === 'traefik' ? 'is-selected' : ''}>
                      <input
                        type="radio"
                        name="deployment-proxy"
                        checked={deploymentDraft.proxy === 'traefik'}
                        onChange={() => updateDeploymentDraft('proxy', 'traefik')}
                      />
                      <strong>Existing Traefik</strong>
                      <small>Traefik owns 80/443 and forwards both hostnames internally.</small>
                    </label>
                  </div>
                  <label>
                    ...
                    <input
                      type="email"
                      value={deploymentDraft.certificateEmail}
                      onChange={(event) =>
                        updateDeploymentDraft('certificateEmail', event.target.value)
                      }
                      placeholder="admin@client.com"
                      autoComplete="email"
                    />
                    <small>Used by the certificate authority for renewal notices.</small>
                  </label>
                </div>
              ) : null}
              ...
              {deploymentStep === 3 ? (
                <div className="admin-deployment-panel">
                  <div>
                    <span className="admin-deployment-eyebrow">Step 03</span>
                    <h3>Choose the first runtime mode</h3>
                    <p>
                      Start with Test Mode for a safe demonstration. Switch to Live
                      Mode only after the server secrets, payments, and databases are ready.
                    </p>
                  </div>
                  <div className="admin-deployment-choice-grid">
                    <label className={deploymentDraft.environmentMode === 'test' ? 'is-selected is-test' : ''}>
                      <input
                        type="radio"
                        name="deployment-mode"
                        checked={deploymentDraft.environmentMode === 'test'}
                        onChange={() => updateDeploymentDraft('environmentMode', 'test')}
                      />
                      <strong>Test Mode</strong>
                      <small>Safe demo data and test integrations. Recommended for first launch.</small>
                    </label>
                    <label className={deploymentDraft.environmentMode === 'live' ? 'is-selected is-live' : ''}>
                      <input
                        type="radio"
                        name="deployment-mode"
                        checked={deploymentDraft.environmentMode === 'live'}
                        onChange={() => updateDeploymentDraft('environmentMode', 'live')}
                      />
                      <strong>Live Mode</strong>
                      <small>Real business data, AI calls, and payment integrations.</small>
                    </label>
                  </div>
                  <div className="admin-deployment-warning" role="note">
                    <strong>Secrets stay on the server</strong>
                    <span>
                      The wizard never stores database passwords or API keys in this browser.
                      Configure them in <code>deploy/.env.production</code> on the VPS.
                    </span>
                  </div>
                </div>
              ) : null}
              ...
              {deploymentStep === 4 ? (
                <div className="admin-deployment-panel admin-deployment-review">
                  <div>
                    <span className="admin-deployment-eyebrow">Step 04</span>
                    <h3>Review and prepare the deployment</h3>
                    <p>
                      Both domains and the proxy plan below are used to create a server-side
                      environment template. Review it before copying it to the VPS.
                    </p>
                  </div>
                  <dl className="admin-deployment-summary">
                    <div><dt>Public site</dt><dd>{deploymentDraft.publicDomain}</dd></div>
                    <div><dt>Dashboard</dt><dd>{deploymentDraft.dashboardDomain}</dd></div>
                    <div><dt>HTTPS email</dt><dd>{deploymentDraft.certificateEmail}</dd></div>
                    <div><dt>Proxy</dt><dd>{deploymentDraft.proxy === 'caddy' ? 'Caddy direct' : 'Existing Traefik'}</dd></div>
                    <div><dt>Mode</dt><dd>{deploymentDraft.environmentMode === 'test' ? 'Test Mode' : 'Live Mode'}</dd></div>
                  </dl>
                  <label>
                    <span>Generated environment template</span>
                    <textarea
                      className="admin-deployment-code"
                      readOnly
                      value={buildDeploymentEnvTemplate(deploymentDraft)}
                      rows={13}
                    />
                  </label>
                  <div className="admin-deployment-command">
                    <code>cp deploy/.env.production.example deploy/.env.production</code>
                    <code>./deploy/deploy.sh</code>
                  </div>
                  <div className="admin-inline-actions">
                    <button type="button" className="admin-button-secondary" onClick={downloadDeploymentEnv}>
                      Download env template
                    </button>
                    <button type="button" className="admin-button-secondary" onClick={copyDeploymentCommand}>
                      {deploymentCopied ? 'Commands copied' : 'Copy commands'}
                    </button>
                    <button type="button" className="admin-button-secondary" onClick={resetDeploymentWizard}>
                      Clear draft
                    </button>
                  </div>
                </div>
              ...
              {deploymentNotice ? (
                <p className="admin-management-notice" role="status">{deploymentNotice}</p>
              ) : null}
              <div className="admin-deployment-actions">
                <button
                  type="button"
                  className="admin-button-secondary"
                  onClick={() => setDeploymentStep((current) => (current > 1 ? (current - 1) as DeploymentWizardStep : current))}
                  disabled={deploymentStep === 1}
                >
                  Back
                </button>
                {deploymentStep < 4 ? (
                  <button type="button" className="admin-button-primary" onClick={advanceDeploymentStep}>
                    Continue
                  </button>
                ) : null}
              </div>
