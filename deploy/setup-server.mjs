...
function saveProxyOverride(proxy) {
  ...
  writeFileSync(
    overrideFile,
    `# Generated by OpenBcon Bootstrap Setup.\n# Traefik must route both the public and dashboard hosts to 127.0.0.1:8080.\nservices:\n  caddy:\n    ports:\n      - "127.0.0.1:8080:80"\n    volumes:\n      - "\${OPENBCON_ROOT:-/opt/openbcon}/deploy/Caddyfile.http:/etc/caddy/Caddyfile:ro"\n      - caddy-data:/data\n      - caddy-config:/config\n`,
    { mode: 0o600 },
  )
}

function buildEnvironment(input) {
  const publicDomain = (input.publicDomain || input.domain).trim().toLowerCase()
  const dashboardDomain = (input.dashboardDomain || `dashboard.${publicDomain}`).trim().toLowerCase()
  const publicUrl = publicDomain === 'localhost' ? 'http://localhost:5173' : `https://${publicDomain}`
  const dashboardUrl = dashboardDomain === 'localhost' ? 'http://localhost:5173' : `https://${dashboardDomain}`
  let text = readBaseEnv()
  text = text.replaceAll('dashboard.your-domain.example', dashboardDomain)
  text = text.replaceAll('your-domain.example', publicDomain)
  text = setEnvValue(text, 'PUBLIC_DOMAIN', publicDomain)
  text = setEnvValue(text, 'DASHBOARD_DOMAIN', dashboardDomain)
  text = setEnvValue(text, 'DOMAIN', publicDomain)
  ...
  text = setEnvValue(text, 'PUBLIC_SITE_URL', publicUrl)
  text = setEnvValue(text, 'DASHBOARD_APP_URL', dashboardUrl)
  text = setEnvValue(text, 'PUBLIC_APP_URL', dashboardUrl)
  text = setEnvValue(text, 'CORS_ORIGIN', `${publicUrl},${dashboardUrl}`)
...
function configuredDomains() {
  if (setupInput?.publicDomain || setupInput?.domain) {
    const publicDomain = (setupInput.publicDomain || setupInput.domain).trim().toLowerCase()
    return {
      publicDomain,
      dashboardDomain: (setupInput.dashboardDomain || `dashboard.${publicDomain}`).trim().toLowerCase(),
    }
  }
  if (!existsSync(envFile)) return { publicDomain: '', dashboardDomain: '' }
  const text = readFileSync(envFile, 'utf8')
  const publicDomain = getEnvValue(text, 'PUBLIC_DOMAIN', getEnvValue(text, 'DOMAIN')).trim().toLowerCase()
  return {
    publicDomain,
    dashboardDomain: getEnvValue(text, 'DASHBOARD_DOMAIN', `dashboard.${publicDomain}`).trim().toLowerCase(),
  }
}
...
async function statusPayload() {
  ...
  const { publicDomain, dashboardDomain } = configuredDomains()
  if (!publicDomain) return { ...deployment, dns: { status: 'pending', message: 'Save the deployment form to check both domains.' }, application: { status: 'pending', message: 'Waiting for configuration.' } }
  const [publicDns, dashboardDns] = await Promise.all([checkDns(publicDomain), checkDns(dashboardDomain)])
  const [publicApplication, dashboardApplication] = await Promise.all([
    checkApplication(publicDomain, publicDns),
    checkApplication(dashboardDomain, dashboardDns),
  ])
  const statuses = [publicDns.status, dashboardDns.status]
  const dnsStatus = statuses.includes('pending') ? 'pending' : statuses.includes('warning') ? 'warning' : 'ready'
  const appStatuses = [publicApplication.status, dashboardApplication.status]
  const applicationStatus = appStatuses.includes('warning') ? 'warning' : appStatuses.includes('pending') ? 'pending' : 'ready'
  return {
    ...deployment,
    configurationSaved: existsSync(completionFile),
    domain: publicDomain,
    dashboardDomain,
    publicUrl: publicDomain === 'localhost' ? 'http://localhost:5173' : `https://${publicDomain}`,
    dashboardUrl: dashboardDomain === 'localhost' ? 'http://localhost:5173' : `https://${dashboardDomain}`,
    dns: { status: dnsStatus, message: `Public site (${publicDomain}): ${publicDns.message} Dashboard (${dashboardDomain}): ${dashboardDns.message}` },
    application: { status: applicationStatus, message: `Public site: ${publicApplication.message} Dashboard: ${dashboardApplication.message}` },
  }
}
...
<body>
  <main>
    ...
    <form id="setup-form">
      <div class="grid">
        <label>Public site domain<input name="publicDomain" placeholder="client.example.com" required spellcheck="false"><small>Point an A record for this domain to <strong>${escapeHtml(serverAddress)}</strong> first.</small></label>
        <label>Dashboard domain<input name="dashboardDomain" placeholder="dashboard.client.example.com" required spellcheck="false"><small>Point a second A record for this domain to <strong>${escapeHtml(serverAddress)}</strong> first.</small></label>
  ...
  <script>
    ...
    const statusPanelMarkup = '<section class="status-panel" aria-live="polite">' +
      ...
      '<div class="status-list">' +
        ...
        '<div class="status-row" id="dns-row"><span class="status-dot"></span><div><strong>Domain connections</strong><small>Checking public site and dashboard DNS resolution.</small></div><span class="status-state">Checking</span></div>' +
        ...
        '<div class="status-row" id="health-row"><span class="status-dot"></span><div><strong>HTTPS and API health</strong><small>Waiting for both public and dashboard URLs.</small></div><span class="status-state">Waiting</span></div>' +
      '</div>' +
    ...
    async function refreshStatus() {
      try {
        ...
        if (completed) welcomeButton.href = payload.dashboardUrl;
        ...
        else if (completed) nextStep.textContent = 'Save the database credentials above. Then select Welcome to OpenBcon to open the dashboard at ' + payload.dashboardUrl + '. The public site is ' + payload.publicUrl + '. Close inbound TCP 8090 in the VPS firewall.';
        else if (payload.dns.status !== 'ready') nextStep.textContent = 'Point A records for ' + payload.domain + ' and ' + payload.dashboardDomain + ' to ' + serverAddress + '. DNS changes may take a few minutes.';
...
const server = http.createServer(async (request, response) => {
  ...
  if (request.method === 'POST' && url.pathname === '/api/setup') {
    ...
    try {
      ...
      if (!isValidDomain(input.publicDomain || input.domain || '')) throw new Error('Enter a valid public site domain.')
      if (!isValidDomain(input.dashboardDomain || '')) throw new Error('Enter a valid dashboard domain.')
